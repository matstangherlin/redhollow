extends RefCounted
class_name ChainPenitentAssetValidator

## Validates Chain Penitent spritesheets against the 38×58 beta contract.

static func validate_pilot_set() -> Dictionary:
	return validate_animation_ids(ChainPenitentAnimationContract.PILOT_ANIMATION_IDS)
...
static func validate_animation_ids(animation_ids: PackedStringArray) -> Dictionary:
	...
	return {
		...
		"approved_frame_size_doc": ChainPenitentAnimationContract.APPROVED_FRAME_SIZE_DOC,
		"gameplay_collision_size": ChainPenitentAnimationContract.GAMEPLAY_COLLISION_SIZE,
		"visual_contract": ChainPenitentAnimationContract.get_visual_contract_summary(),
		"results": results,
		"found": found,
		"missing": missing,
		"passed": passed,
...
static func validate_sheet(anim_id: StringName) -> Dictionary:
	...
	var sheet_path := ChainPenitentAnimationContract.resolve_sheet_path(anim_id)
	var entry := {
		"animation": String(anim_id),
		"expected_file": expected_file,
		"sheet_path": ChainPenitentAnimationContract.get_sheet_path(anim_id),
		"found": false,
		"passed": false,
	...
	var frame_size := ChainPenitentAnimationContract.APPROVED_FRAME_SIZE
...
static func format_report(report: Dictionary) -> String:
	...
	lines.append(
		"Chain Penitent asset validation — approved frame %dx%d"
