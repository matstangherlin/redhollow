extends RefCounted
class_name VermiliteGunslingerSpriteFramesBuilder
...
static func build_for_profile(profile: EnemyVisualProfile) -> SpriteFrames:
	...
	if profile.is_pilot_profile() and profile.use_procedural_pilot_frames:
		return VermiliteGunslingerPlaceholderFactory.create_pilot_sprite_frames()
...
static func _ensure_clips(frames: SpriteFrames) -> SpriteFrames:
	for anim_id in VermiliteGunslingerAnimationContract.PILOT_ANIMATION_IDS:
		if frames.has_animation(anim_id):
			continue
		VermiliteGunslingerAnimationContract.warn_missing_once("missing clip", String(anim_id))
		_merge_proc(frames, anim_id)
	return frames


static func _try_sheet(frames: SpriteFrames, anim_id: StringName) -> bool:
	var path := VermiliteGunslingerAnimationContract.resolve_sheet_path(anim_id)
	if path.is_empty():
		return false
	var image := Image.new()
	if image.load(ProjectSettings.globalize_path(path)) != OK:
		return false
	var fs := VermiliteGunslingerAnimationContract.APPROVED_FRAME_SIZE
...
static func _merge_proc(frames: SpriteFrames, anim_id: StringName) -> void:
	var src := VermiliteGunslingerPlaceholderFactory.create_single_clip(anim_id)
