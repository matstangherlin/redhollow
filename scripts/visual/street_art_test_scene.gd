...
func _ready() -> void:
	_art_area.set_presentation_mode(Spec.MODE_NORTH_STAR)
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("debug_toggle"):
		...
		_update_camera_for_mode()
		_update_mode_label()
		get_viewport().set_input_as_handled()


func _update_camera_for_mode() -> void:
	if _mode == Spec.MODE_FINAL_CANDIDATE:
...
func _update_mode_label() -> void:
	if _mode_label == null:
		return
	_mode_label.text = (
		"Street Art Test — %s | F cycle 3 modes | P perf | ' region state"
