...
### Arena — spawn durante physics flush (KI-002)

- **Arquivo:** `combat_arena_controller.gd`
- **Problema:** `Can't change this state while flushing queries` (allowlist headless; **não corrigido** em produção).
- **Direção:** spawn/ativação deferred.
...
| KI-102 | Auto-load off (`game.gd`) — decisão D-013 |
...
3. P1: decisão auto-load beta
4. P2: panic unlock, build smoke, hitstop via serviço
5. Arte/conteúdo em paralelo (restrições acima)
